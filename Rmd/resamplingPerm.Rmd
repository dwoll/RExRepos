...
[`coin`](https://cran.r-project.org/package=coin), [`e1071`](https://cran.r-project.org/package=e1071)
...
tDepDfW <- data.frame(DVpre, DVpost)          # wide format
tDepDfL <- data.frame(DV=c(DVpre, DVpost),    # long format
                      IV=factor(rep(0:1, each=N), labels=c("pre", "post")))
```

```{r}
library(coin)
oneway_test(DV ~ IV | id, alternative="less", distribution=approximate(nresample=9999),
            data=tDepDfL)
```

```{r}
t.test(Pair(DVpre, DVpost) ~ 1, alternative="less", data=tDepDfW)$p.value
```
...
Packages [`resample`](https://cran.r-project.org/package=resample) and [`vegan`](https://cran.r-project.org/package=vegan) provide more ways to implement flexible permutation strategies for various designs.
